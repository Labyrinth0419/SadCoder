...
- 本轮 UI pass 已补强 Chat 主画布权重和 TUI 状态 marker，侧栏开关固定为左上三横线，timeline turn 作为主内容渲染并有 widget 测试覆盖；斜杠命令预览仍保留为输入附近的轻提示，不再抢占默认对话信息架构。
...
- 本轮 UI pass 已继续压缩 Files 顶部和工具栏密度，文件搜索栏同时限制高度和宽度，预览面板保留主区域 status page / opened file 的单一主体语义，并用轻量阴影保留层次而非完全扁平化。
