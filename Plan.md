...
- 已落地 per-host thread item cache reader 接入：`ThreadItemCacheStore` 按 profile/thread 隔离保存 item summaries 和分页游标，`local_data` schema 已补 `item_cache.profile_id` 与 profile/thread 索引；`CodexSessionStateController.threadItemListReader` 已通过缓存 decorator 保存/回退 canonical thread item 读取。
- 后续仍需完整多 host 同时连接架构：`HostSessionManager` 已作为基础控制器引入，但断线事件回填、item cache 的 timeline/offline UI recovery 和后台保活策略还需要继续拆分完善。
