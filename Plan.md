...
- 已落地 per-host thread item cache 数据层：`ThreadItemCacheStore` 按 profile/thread 隔离保存 item summaries 和分页游标，`local_data` schema 已补 `item_cache.profile_id` 与 profile/thread 索引；后续可接入断线 item 回填和 timeline 恢复。
- 后续仍需完整多 host 同时连接架构：`HostSessionManager` 已作为基础控制器引入，但断线事件回填、item cache 的 UI/recovery 接入和后台保活策略还需要继续拆分完善。
