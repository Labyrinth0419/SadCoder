...
- 已落地 Settings Diagnostics 的 App version 展示：Diagnostics 顶部会显示移动端包版本 `sadcoderMobileAppVersion`，并与 agent version、Codex version 放在同一只读诊断入口；app-server initialize 仍使用独立的 `sadcoderMobileClientVersion`，避免把发布包版本和协议 client version 混用。
