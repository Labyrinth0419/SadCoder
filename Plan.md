...
- 已覆盖 `en-US` / `zh-CN` 基础资源完整性、数字/日期/文件大小格式化和斜杠命令说明本地化；Chat 中 `/rollout`、`/test-approval`、`/debug-config` 未知 layer 标签和 `/plugins` 本地版本标签等可见诊断/摘要文本已移入资源文件，避免 debug/diagnostic/summary 路径继续硬编码英文 UI。
