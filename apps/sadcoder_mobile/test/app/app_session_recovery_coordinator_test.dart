...
void main() {
  ...
  test(
    'uses conservative turn backfill when recovery hint reports cursor gap',
    () async {
      final turnListReader = _RecordingThreadTurnListReader.pages([
        ThreadTurnsPage(
          turns: [
  ...
  test(
    'uses conservative item backfill when recovery hint reports cursor gap',
    () async {
      final itemListReader = _RecordingThreadItemListReader.pages([
        ThreadItemsPage(
          items: [
...
class _Fixture {
  _Fixture({
    ...
    coordinator = AppSessionRecoveryCoordinator(
      ...
      threadTimelineCursorProvider:
          timelineCursor == null || forceConservativeBackfill
          ? null
          : (threadId) async => timelineCursor,
      threadRecoveryHintProvider: forceConservativeBackfill
          ? (threadId) async => ThreadRecoveryHint(
